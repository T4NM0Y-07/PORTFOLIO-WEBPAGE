...
const certificateModal = document.getElementById("documentModal");
const certificateFrame = document.getElementById("documentFrame");
const modalTitle = document.getElementById("modalTitle");

const portfolioData = {
  email: "tanmoyhere07@gmail.com",
};
...
function updatePointer(event) {
  ...
  // subtle hero parallax via CSS vars (avoids clobbering transitions)
  const px = Math.round((pointer.x - 0.5) * 18);
  const py = Math.round((pointer.y - 0.5) * 8);
  if (heroCopy) {
    heroCopy.style.setProperty('--hero-px', `${px}px`);
    heroCopy.style.setProperty('--hero-py', `${py}px`);
  }
}
...
const docButtons = document.querySelectorAll("[data-document-src]");
if (docButtons && docButtons.length) {
  docButtons.forEach((button) => {
    button.addEventListener("click", () => {
      openCertificate(button.dataset.documentTitle, button.dataset.documentSrc);
    });
  });
}
...
if (canvas && ctx) {
  ...
  console.warn("Canvas or context not available — visuals disabled.");
}

// Remove preload class after full load so entrance animations run smoothly
